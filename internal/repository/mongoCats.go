...
func NewRepositoryMongo(mongoDB *mongo.Database) *RepositoryMongo {
...
func (rep *RepositoryMongo) SelectC(c context.Context, id string) (*model.Record, error) {
	...
	err := rep.collection.FindOne(c, bson.M{"_id": id}).Decode(&rec)
...
func (rep *RepositoryMongo) UpdateC(c context.Context, rec *model.Record) error {
	if r, err := rep.collection.UpdateOne(c, bson.M{"id": rec.ID}, bson.M{"$set": bson.M{"name": rec.Name, "type": rec.Type}}); err != nil {
		return err
...
func (rep *RepositoryMongo) DeleteC(c context.Context, id string) error {
	if r, err := rep.collection.DeleteOne(c, bson.M{"id": id}); err != nil {
		return err
